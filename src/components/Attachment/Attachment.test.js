import { render, fireEvent, screen, waitFor, fail } from '@testing-library/react';
...
it('should render a delete attachment icon, attachment title and download attachment icon', () => {
  ...
  const { queryByTitle, queryByText } = render(<Attachment attachment={mockAttachmentData} />);
...
it('should render a label value not appending the three dots at the end', () => {
  ...
  const { queryByText } = render(<Attachment attachment={mockAttachmentData} />);
