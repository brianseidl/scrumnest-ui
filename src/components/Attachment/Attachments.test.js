...
it('should render a list of 2 attachments and contain a file upload field', () => {
  ...
  const numOfAttachments = attachmentsComp.container.querySelector('#attachments-container').childElementCount;
  expect(numOfAttachments).toBe(2);

  const fileUploadField = attachmentsComp.container.querySelector('#file-input');
...
it('should not be able to upload a file because of file size', () => {
  ...
  expect(attachmentsComp.instance().acceptableFileSize(mockFileObj)).toBeFalsy();
});
...
it('should return the correct file extension', () => {
  ...
  const attachmentsComp = shallow(<Attachments attachments={data} />);
  expect(attachmentsComp.instance().getFileExtension(mockFileObj.name)).toBe('.jpg');
  ...
  expect(attachmentsComp.instance().getFileExtension(mockFileObj2.name)).toBe('.docx');
