...
it('should render a comment with the appropriate HTML element values', () => {
  ...
  const { queryByText, queryByTitle } = render(<Comment comment={mockCommentObj} />);
...
describe('typing in a comment', () => {
  ...
  it('should show the value the user typed in the textarea', () => {
    const { queryByText, getByRole } = render(<Comment comment={mockCommentObj} />);
    ...
    expect(getByRole('textbox')).toHaveValue(`${mockCommentObj.content}${changedCommentText}`);
  });
})
...
it('should remove the save and discard button if enabled is set to false and display the date', () => {
  ...
  const { queryByTitle, queryByText } = render(<Comment comment={mockCommentObj} />);
